...
class DataTransformationWorkflow:
    ...
    def create_sites_catalog(self):
        ...
        shared_scratch_dir = os.path.join(self.wf_dir, "scratch-eclair")
        eclair = Site("eclair")\
                    .add_directories(
                        Directory(Directory.SHARED_SCRATCH, shared_scratch_dir)
                            .add_file_servers(FileServer("file://" + shared_scratch_dir, Operation.ALL))
        ...
        eclair.add_env(key="PEGASUS_HOME", value="/usr")
        eclair.add_env(key="CONDA_PREFIX", value="/nas/home/georgpap/miniconda3/envs/pegasus")
        eclair.add_env(key="PATH", value="/nas/home/georgpap/miniconda3/envs/pegasus/bin:/nas/home/georgpap/miniconda3/condabin:/usr/local/bin:/usr/bin:/usr/local/sbin:/usr/sbin:/nas/home/georgpap/.local/bin:/nas/home/georgpap/bin")
        eclair.add_env(key="CONDA_DEFAULT_ENV", value="pegasus")

        self.sc.add_sites(local, condorpool, eclair)
    ...
    def create_transformation_catalog(self, target_site="condorpool"):
        ...
        base_container = Container("base-container",
            ...
            image=f"file://{self.wf_dir}/container/somospie-gdal-netcdf_latest.sif",
            image_site="local",
            mounts=["/nas/home/georgpap:/nas/home/georgpap"] if target_site == "eclair" else []
        )
    ...
    # --- Plan -----------------------------------------------------------------------
    def plan_workflow(self, submit=False, site="condorpool"):
        try:
            self.wf.plan(sites=[site], output_dir="output", dir="submit", output_sites=["local"], submit=submit)
            if submit:
                self.wf.wait()
...
def main():
    ...
    parser.add_argument("--site", metavar="STR", type=str, default="condorpool", choices=["condorpool", "eclair"], help="Execution Site", required=False)
    ...
    workflow.create_transformation_catalog(target_site=args.site)
    ...
    if args.plan or args.submit:
        workflow.plan_workflow(submit=args.submit, site=args.site)
